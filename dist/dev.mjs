...
function hasProperty(node, field) {
  const value = typeof field === "string" && isNode(node) && node.type === "element" && node.properties && own2.call(node.properties, field) && node.properties[field];
...
// src/node/vitePlugins.ts
function createVitePlugins(config, restart) {
  return [
    PluginIndexHtml(),
    pluginReact(),
